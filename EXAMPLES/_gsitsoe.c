...
void main ()
{
	...
	void gssitisolerb(real_t **, int, real_t [], real_t []);
	int i,j;
	real_t **a,b[5],aux[14];
	...
	printf("Sign(Det) =%3.0f\nNumber of eliminations =%3.0f\n"
			...
			"Norm calculated inverse matrix : = %e\n"
			"Upper bound for the relative error : = %e\n"
			"Norm residual vector = %e\n",
			aux[1],aux[3],aux[5],aux[7],aux[9],aux[11],aux[13]);
